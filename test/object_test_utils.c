...
void test_object_object(object_object *obj, object_object *expected) {
    if (!obj) {
        fprintf(stderr, "Error: NULL obj passed to test_object_object\n");
        abort();
    }
    if (!expected) {
        fprintf(stderr, "Error: NULL expected passed to test_object_object\n");
        abort();
    }
    if (expected->type < OBJECT_INT || expected->type > OBJECT_COMPILED_FUNCTION) {
        fprintf(stderr, "Error: Invalid expected->type: %d\n", expected->type);
        abort();
    }
    ...
    if (expected->type == OBJECT_INT) {
        ...
        if (!expected_int) {
            fprintf(stderr, "Error: NULL expected_int in OBJECT_INT branch\n");
            abort();
        }
        ...
        if (!expected_bool) {
            fprintf(stderr, "Error: NULL expected_bool in OBJECT_BOOL branch\n");
            abort();
        }
        ...
        if (!expected_str_obj || !expected_str_obj->value) {
            fprintf(stderr, "Error: NULL expected_str_obj or value in OBJECT_STRING branch\n");
            abort();
        }
        ...
        if (!actual_err || !expected_err || !actual_err->message || !expected_err->message) {
            fprintf(stderr, "Error: NULL object_error or message in OBJECT_ERROR branch\n");
            abort();
        }
